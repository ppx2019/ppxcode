...
# 提取出 电池组电压列表
rdd_data=data.select('resd_subsystem_temperature_list','obd_time').rdd.map(lambda x:eval(x[0])[0]['subsystem_probe_temperature_list']).collect()
print(rdd_data)
data2=spark.createDataFrame(rdd_data)
type(data2)
data2.toPandas()
data3.to_svc('/Users/ppx/Desktop/subsystem_probe_temperature_list.txt', sep='\t', index=False)

# 转化数据格式
row1=data.select("resd_subsystem_voltage_list").rdd.map(lambda x:(eval(x[0])[0])).collect()
print(row1)
for l in row1:
    print(l)
